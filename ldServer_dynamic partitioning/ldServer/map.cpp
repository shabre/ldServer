...
int map::getNumOfZone(int serverNum){
    int res=0;
    for(int i=0; i<numOfZone; i++){
...
int map::getShiftBoundCost(int srcServer, int dstServer, int zoneNum){
    /*
    int beforeBound=getBoundCost(srcServer);
    int numOfZone=getNumOfZone(srcServer);
    int inner=0, outer=0;
    int x=zoneNum/3;
    int y=zoneNum%3;
    
    if(x-1>=0){
        if(zoneServer[(x-1)*3+y]==srcServer)
            inner+=1;
        else
            outer+=1;
    }
    if(y-1>=0){
        if(zoneServer[x*3+y-1]==srcServer)
            inner+=1;
        else
            outer+=1;
    }
    ...
    int afterBound=beforeBound+inner-outer;
    int beforeCost=beforeBound-numOfZone;
    int afterCost=afterBound-numOfZone-1;
